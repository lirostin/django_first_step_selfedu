...
[link](https://proproprogs.ru/modules/nasledovanie-rasshirenie-shablonov)
Рассмотрим механизм расшиерния шаблонов в Jinja, его также называют **наследованием**. Это удобный инструмент, уменьшающий объем дублируемого кода в шаблонах. 
Создадим целостную страницу с **именованными блоками**
1. 
`templates/ex_main.html`
```HTML
<!DOCTYPE html>
<html>
<head>
...
```
### синтаксис именнованного блока
```python
{% block <имя блока> %}  
{% endblock %}
```
Данные блоки используются для создания расширенния базового шаблона страницы `ex_main.html` для создания всех страниц текущего сайта. Расширения или наследование шаблона делается следующим образом. 
2. 
`templates/about.html`
```HTML
# здесь указываем базовый шаблон
{% extends 'ex_main.html' %}

# Здесь указываем информацию которая будет содержаться в именнованых блоках
...
Если шаблон расположен в другой папке: `templates/about.html`
```HTML
{% extends 'layout/default.tpl' %}
```

Если у нас есть похожая информация, на пример первая строчка блока **content** содежит информацию блок **title**.
Здесь self ссылается на объект текущего шаблона, в котором имеется метод title
`templates/about.html`
```HTML
{% extends 'layout/default.tpl' %}
...
### Обращение к блоку базового шаблона 
Обращение к блоку базового шаблона осуществляется через параметр **super**

1. 
`templates/layout/default.tpl
```HTML
<!DOCTYPE html>
<html>
<head>
...
Пропишем в шаблоне базового класса в блоке content ещё один блок -  table_contents
1. 
`templates/layout/default.tpl
```HTML
<!DOCTYPE html>
...
subs = ["Физика", "Информатика", "Математика"]
...
Для примера добавим в базовый шаблон ещё один блок для фомирования элементов спсика. Здесь после именнованного блока item используется ключевое слово `scoped` для оперирования переменными из внешней области видимости.
1. 
`templates/layout/default.tpl
```HTML
...
...
...
```
Переопределим блок item в дочернем шаблоне
`templates/about.html`
```HTML
{% extends 'layout/default.tpl' %}
...
Шаблоны поддерживают вложенное наследование, то есть, формирование итогового дочернего шаблона по цепочке: от базового (корневого) до последнего выходного
![[Pasted image 20220723204626.png]]
